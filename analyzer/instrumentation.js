...
(function() {
  ...
  function _hookMechanics() {
    ...
    const _mechTimer = setInterval(() => {
      ...
      // Spy detection: count spy units in field
      ...
      // Upgrades: monitor G._upgradesBought or count purchased upgrades
      for (let pid = 1; pid <= 2; pid++) {
        const faction = G.factions?.[pid - 1];
        if (!faction) continue;
        const bought = (faction.upgrades || []).filter(u => u.purchased).length;
        if (bought > _upgradesBought[pid - 1]) {
          window.__qa.mechanics.upgrade_purchased += (bought - _upgradesBought[pid - 1]);
          _upgradesBought[pid - 1] = bought;
        }
      }

      // Buff activation: check if any buff is currently active
      for (let i = 0; i < 2; i++) {
        const p = G.players?.[i];
        if (p && p.activeBuff && p.activeBuff.active) {
